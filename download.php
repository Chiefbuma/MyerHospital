        ...
        <main>
            ...
            <div class="todo">
                <div class="todo">
                    ...
                    // $db is your database connection object

                    // Retrieve the from and to dates from the query string or set default values
                    $from_date = isset($_GET['from_date']) ? $_GET['from_date'] : date('Y-m-d');  // default to a very old date
                    $to_date = isset($_GET['to_date']) ? $_GET['to_date'] : date('Y-m-d');  // default to today's date

                    // Check if the download CSV parameter is set
                    if (isset($_POST['download_csv'])) {
                        ...
                        $psychosocialQuery = "
        SELECT psychosocial.*, patient.*
        FROM psychosocial
        LEFT JOIN patient ON psychosocial.id = patient.patient_id
        WHERE psychosocial.visit_date BETWEEN '$from_date' AND '$to_date'
    ";

                        // Execute the query
                        $result = $db->conn->query($psychosocialQuery);
                        $rows = [];
                        while ($row = $result->fetch_assoc()) {
                            $rows[] = $row;
                        }

                        // Send CSV headers and output data
                        header('Content-Type: text/csv');
                        header('Content-Disposition: attachment; filename="psychosocial_data.csv"');
                        $output = fopen('php://output', 'w');
                        fputcsv($output, array_keys($rows[0])); // Write headers
                        foreach ($rows as $row) {
                            fputcsv($output, $row); // Write rows
                        }
                        fclose($output);
                        exit;
                    }
                    ...
                    <head>
                        <meta charset="UTF-8">
                        <meta name="viewport" content="width=device-width, initial-scale=1.0">
                        <title>Psychosocial Assessment</title>
                    ...
                    <body>
                        ...
                        <div class="container-fluid mt-5">
                            <div class="row justify-content-center">
                                <div class="col-md-8">
                                    <div class="form-group d-flex align-items-center">
                                        <!-- From Date Filter -->
                                        <label for="from_date" class="mt-2 me-2">From</label>
                                        <input type="date" id="from_date" value="<?php echo $from_date; ?>" class="form-control me-3">

                                        <!-- To Date Filter -->
                                        <label for="to_date" class="mt-2 me-2">To</label>
                                        <input type="date" id="to_date" value="<?php echo $to_date; ?>" class="form-control">
                                    </div>
                                </div>
                            </div>
                            ...
                            <form method="post">
                                <button type="submit" name="download_csv" class="btn btn-primary mt-3">Download CSV</button>
                            </form>

                            <!-- Psychosocial Table -->
                            <h2 class="mt-5">Psychosocial Assessments</h2>
                                ...
                                <table id="example" class="display" style="width:100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Visit Date</th>
                                            <th>Revenue</th>
                                            <th>Type</th> <!-- To distinguish between Nutrition and Psychosocial -->
                                        </tr>
                                    </thead>
                                    <tbody id="table-body">
                                        <!-- Data will be dynamically inserted here via AJAX -->
                                    </tbody>
                                </table>
                            </div>
                        </div>
                ...
                `;
                            ...
                            // Listen for changes in date input fields
                            document.getElementById('from_date').addEventListener('change', updateTableData);
                            document.getElementById('to_date').addEventListener('change', updateTableData);

                            // Load initial data on page load
                            $(document).ready(function() {
                                updateTableData(); // Load data initially
                            });
                        </script>
