...
try {
    ...
    $from_date = isset($_GET['medication_from_date']) ? $_GET['medication_from_date'] : null;
    $to_date = isset($_GET['medication_to_date']) ? $_GET['medication_to_date'] : null;
    ...
    $condition = ($user_role == 'admin') ? "1=1" : "medication_use.branch_id = ?";
    $params = [];
    $types = '';
    ...
    if ($from_date && $to_date) {
        $condition .= " AND medication_use.visit_date BETWEEN ? AND ?";
        $types .= 'ss';
        $params[] = $from_date;
        $params[] = $to_date;
    } elseif ($from_date) {
        $condition .= " AND medication_use.visit_date >= ?";
        $types .= 's';
        $params[] = $from_date;
    } elseif ($to_date) {
        $condition .= " AND medication_use.visit_date <= ?";
    ...
    if ($report && $report->num_rows > 0) {
        while ($row2 = $report->fetch_assoc()) {
            ...
            echo "<td>" . htmlspecialchars($row2['medication_name']) . "</td>";
            echo "<td>" . htmlspecialchars($row2['cohort']) . "</td>";
            echo "<td>" . htmlspecialchars($row2['no_pills_dispensed']) . "</td>";
            echo "<td>" . htmlspecialchars(number_format($row2['revenue'], 0)) . "</td>";
            echo "</tr>";
        }
    } else {
        echo "<tr><td colspan='7' class='text-center'>No Medication Data Found</td></tr>";
    }
    $stmt->close();
} catch (Exception $e) {
